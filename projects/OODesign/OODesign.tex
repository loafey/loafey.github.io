...
\begin{Parallel}[v]{0.48\textwidth}{0.48\textwidth}
    ...
    \ParallelRText{
        ...
        Could be considered a "Global State". Often critized however as it leads
...
\begin{Parallel}[v]{0.49\textwidth}{0.49\textwidth}
    \ParallelLText{
        ...
        Static methods are a single copy of a method and can not access instance variables but can
        be used without an instance, complete opposite of an instance method which can access instance
        variables and requires an instance.
        ...
        An interface is a form of specification inheritance where what methods a class needs to have
        is defined, similarly an abstract class can do this while also having methods and attributes
        that are inherited. But can be used as a type but neither can be instanced.

        \textbf{Dynamic Binding}\newline
        Also called late binding, and is when you are overriding public methods when using
        inheritance as the used method is decided during runtime.\\
        The opposite of this is static binding which is done with static, private and final methods
        as they cannot be overriden.
    }
    \ParallelRText{
        \textbf{Primitive and referencetypes}\newline
        Primitive types are types such as booleans, integers and floats, and they are stored in the stack.
        Reference types are types such as Classes, which are stored on the heap, and when passing these
        around you pass around the reference to it instead of the data itself.

        \textbf{Alias}\newline
        Alias is when two variables point to the same object.
        (They have a copy of the same reference.)
        \\
        \fbox{A a = new A();\\B b = A;}
...
\begin{Parallel}[v]{0.49\textwidth}{0.49\textwidth}
    ...
    \ParallelRText{
        \textbf{Inheritance}\newline
        Inheriting means making a class a subclass of another, and inherting methods and such from the super.
